...
# Steam is only available on x86_64-linux
# Return empty derivation for other platforms
if pkgs.stdenv.system != "x86_64-linux" then
  pkgs.writeTextFile {
    name = "steam-fix-unsupported";
    text = "Steam is only supported on x86_64-linux";
    destination = "/bin/steam-fix";
    executable = true;
  }
else
